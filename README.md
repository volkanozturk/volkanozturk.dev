...
Every published article carries two images, named in its frontmatter:

| Field | Used on |
|---|---|
| `thumbnail` | the home page and the Writing index |
| `cover` | the top of the article page |

The two are separate compositions of the same concept, each designed for its own
aspect ratio — never one cropped or stretched out of the other. Neither is
...
for anything visual. **[docs/image-crate.txt](docs/image-crate.txt)** is the
portable brief to hand to whoever makes the artwork.
